...
function parseStats(sstr: string): Stats {
    let [pstr, hardlinksStr, owner, group, sizeStr, dateStr, timeStr, name] =
        sstr.match(NWS_RX) || ['-----------']
    return {
        ...parsePerms(pstr),
...
export class FileSystem extends DeviceConnection {
    ...
    async readDir(dir: string, stat?: boolean): Promise<Stats[] | string[]> {
        if (stat) {
            return (await this.adb(`shell ls -al "${dir}"`))
                .split(NL_RX)
                .slice(1, -1)
                .map((v) => parseStats(v))
        } else {
            return (await this.adb(`shell ls "${dir}"`))
                .split(NL_RX)
                .slice(0, -1)
        }
    }
    ...
    async stat(file: string): Promise<Stats> {
        return parseStats(
            (await this.adb(`shell ls -al "${file}"`)).split(NL_RX)[0]
        )
    }
